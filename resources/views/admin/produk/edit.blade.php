<x-app-layout>
    ...
    <div class="py-8 max-w-xl mx-auto">
        <form method="POST" action="{{ route('admin.produk.update', $product->id) }}" enctype="multipart/form-data" class="space-y-4 bg-white p-6 rounded shadow">
            @csrf @method('PUT')
            <div>
            ...
            <div>
                ...
                <select name="status_id" class="w-full px-4 py-2 border rounded focus:outline-none" required>
                    @foreach($statuses as $stat)
                        <option value="{{ $stat->id }}" @selected(old('status_id', $product->status_id) == $stat->id)>{{ $stat->name }}</option>
                    @endforeach
                </select>
                @error('status_id')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
            </div>
            <div>

<x-app-layout>
    ...
    <div class="py-8 max-w-xl mx-auto">
        <form method="POST" action="{{ route('admin.kategori.update', $category->id) }}" class="space-y-4 bg-white p-6 rounded shadow">
            @csrf @method('PUT')
            <div>
                <label class="block mb-1 font-semibold">Nama Kategori</label>
                <input type="text" name="name" class="w-full px-4 py-2 border rounded focus:outline-none" value="{{ old('name', $category->name) }}" required>

<x-app-layout>
    ...
    <div class="py-8 max-w-xl mx-auto">
        <form method="POST" action="{{ route('admin.artikel.update', $article->id) }}" enctype="multipart/form-data" class="space-y-4 bg-white p-6 rounded shadow">
            @csrf @method('PUT')
            <div>
                <label class="block mb-1 font-semibold">Judul</label>
                <input type="text" name="title" class="w-full px-4 py-2 border rounded focus:outline-none" value="{{ old('title', $article->title) }}" required>
            ...
            <div>
                ...
                <textarea name="content" rows="6" class="w-full px-4 py-2 border rounded focus:outline-none" required>{{ old('content', $article->content) }}</textarea>

        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 md:p-8 border border-gray-100">
            <form method="GET" action="{{ route('produk.index') }}" class="flex flex-col md:flex-row gap-4">
                ...
                <div class="md:w-1/4 relative">
                    ...
                    <select name="kategori" class="w-full pl-11 pr-10 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-gray-800 appearance-none cursor-pointer">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    ...
                    <div class="group bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden flex flex-col h-full transform hover:-translate-y-1">
                        <div class="relative aspect-square overflow-hidden bg-gray-100">
                            ...
                            @if($product->status->name != 'approved')
                                <span class="absolute top-2 left-2 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider z-10">
                            ...
                            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                <a href="{{ route('produk.detail', $product->id) }}" class="px-4 py-2 bg-white text-gray-900 rounded-lg font-semibold text-sm transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 shadow-lg">
                                    Lihat Detail
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="p-4 flex-1 flex flex-col">
                            <div class="mb-2">
                                <span class="text-[10px] uppercase tracking-wider text-blue-600 font-bold bg-blue-50 px-2 py-1 rounded-md">
                                    {{ $product->category->name ?? 'Umum' }}
                                </span>
                            </div>
                            <h4 class="font-bold text-gray-900 mb-1 line-clamp-2 text-sm group-hover:text-blue-600 transition-colors leading-snug">
                                {{ $product->name }}
                            </h4>
                ...
                @empty
                    <div class="col-span-full flex flex-col items-center justify-center py-16 text-center bg-white rounded-2xl border border-gray-100 shadow-sm">
                        ...
                        <a href="{{ route('produk.index') }}" class="px-6 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-medium shadow-lg shadow-blue-500/30 text-sm">
                            Reset Pencarian
                        </a>
                    </div>
                @endforelse
